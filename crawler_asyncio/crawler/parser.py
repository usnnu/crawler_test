...
from crawler.request_t import fetch
...
class BaseParser(object):
    ...
    async def execute_url(self, url, spider, session, semphore):
        html = await fetch(url, spider, session, semphore)
...
class XpathParser(BaseParser):
    def abstract_urls(self, html):
        page_data = etree.HTML(html)
        urls = page_data.xpath(self.rule)
        return urls
